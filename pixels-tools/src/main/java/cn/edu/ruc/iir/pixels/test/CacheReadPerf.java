...
public class CacheReadPerf {
        ...
        try {
            ...
            for (Path path : cachedPaths) {
                for (int i = 0; i < cachedColumnlets.size(); i++) {
                    ...
                    PixelsCacheIdx pixelsCacheIdx = cacheReader.search(path.toString(), Short.parseShort(columnletIdSplits[0]),
    ...
    static class CacheReader implements Runnable {
        ...
        public void run() {
            ...
            System.out.println("[read]: " + readSize + "," + (readEnd - readStart) + "," + (readSize * 1.0 / (readEnd - readStart)));
        }
    }
}
