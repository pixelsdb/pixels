                            ...
                            std::shared_ptr<PixelsBitMask> filterMask)
{
  ...
  if (encoding->kind() == pixels::fb::EncodingKind_RUNLENGTH)
  {
    for (int i = 0; i < size; i++)
    {
      ...
      {
        columnVector->longVector[outputIndex] = decoder->next();
      }
      elementIndex++;
    }
  } else
  {
    ...
    for (int i = 0; i < size; ++i)
    {
      ...
      uint64_t value = 0;
      for (int byteIndex = 0; byteIndex < sizeof(int64_t); ++byteIndex)
      {
        const int shift = littleEndian ? byteIndex * 8 : (7 - byteIndex) * 8;
        value |= static_cast<uint64_t>(input->get()) << shift;
      }
      columnVector->longVector[outputIndex] = static_cast<int64_t>(value);
      ++elementIndex;
    }
  }
}
