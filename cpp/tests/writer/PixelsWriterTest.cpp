...
TEST_F(PIXELS_WRITER_TEST, DISABLED_SINGLE_STRING)
{
    ...
    while(true) {
        ...
        for(int i = 0; i < rb->rowCount; i++) {
            ...
            std::string actual = v->vector[i].ToString();
...
TEST_F(PIXELS_WRITER_TEST, DISABLED_MULTI_COLUMN)
{
    ...
    while(true) {
        ...
        for(int i = 0; i < rb->rowCount; i++) {
            ...
            std::string actual = rc->vector[i].ToString();
            EXPECT_EQ(actual, expected);
            count++;
        }
    }
...
{
    // Write multiple row groups and files containing different data types.
    int total_rows = 50;
    int rg_size = 10;
    ...
    auto verify_comp_file = [&](const std::string& path, int expected_start) {
        ...
        while(true) {
            ...
            for(int i = 0; i < rb->rowCount; i++) {
                ...
                std::string actual_str = rv3->vector[i].ToString();
                EXPECT_EQ(actual_str, expected_str);

                EXPECT_EQ(rv4->dates[i], expected_val);
                EXPECT_EQ(rv5->times[i], (long)expected_val * 1000);
                EXPECT_EQ(reinterpret_cast<int64_t*>(rv6->vector)[i],
                          static_cast<int64_t>(expected_val));
