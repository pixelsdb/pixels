...
#include <string.h>
#include <byteswap.h>
#include "../include/io_pixelsdb_pixels_cache_legacy_utils_RadixTreeDumper.h"
...
{
    if (currentNodeOffset > max_offset)
    { max_offset = currentNodeOffset; }
...
Java_io_pixelsdb_pixels_cache_utils_RadixTreeDumper_nativeTraverse(JNIEnv
*env,
jobject obj
)
{
...
char keyBuf[12] = {0};
dfs(indexFile, currentNodeOffset, keyBuf,
0, out);
fclose(out);
