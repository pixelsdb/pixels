 ...
 * @date 07/05/2022
 */
public class BroadcastJoinInvoker
{
    private static final String BROADCAST_JOIN_WORKER_NAME = "BroadcastJoinWorker";

    private BroadcastJoinInvoker() { }

    public static CompletableFuture<JoinOutput> invoke(BroadcastJoinInput input)
    {
        String inputJson = JSON.toJSONString(input);
        SdkBytes payload = SdkBytes.fromUtf8String(inputJson);

        InvokeRequest request = InvokeRequest.builder()
                .functionName(BROADCAST_JOIN_WORKER_NAME)
                .payload(payload)
