...
{
    private final Logger log = LogManager.getLogger(ScanStreamInvoker.class);
...
//        log.info(String.format("invoke ScanInput: %s", JSON.toJSONString(input, SerializerFeature.PrettyFormat, SerializerFeature.DisableCircularReferenceDetect)));
        ListenableFuture<TurboProto.WorkerResponse> future = Vhive.Instance().getAsyncClient().scanStreaming((ScanInput) input);
