...
public class AggregationStreamInvoker extends VhiveInvoker
{
    private final Logger log = LogManager.getLogger(AggregationStreamInvoker.class);

    protected AggregationStreamInvoker(String functionName)
    {
        super(functionName);
    }
...
//        log.info(String.format("invoke AggregationInput: %s", JSON.toJSONString(input, SerializerFeature.PrettyFormat, SerializerFeature.DisableCircularReferenceDetect)));
        ListenableFuture<TurboProto.WorkerResponse> future = Vhive.Instance().getAsyncClient().aggregationStreaming((AggregationInput) input);
