...
import io.pixelsdb.pixels.turbo.planner.plan.physical.input.BroadcastJoinInput;
        ...
        while (!leftInputs.isEmpty())
        {
            for (Iterator<InputInfo> it = leftInputs.iterator(); it.hasNext(); )
            {
                ...
                try (PixelsReader pixelsReader = WorkerCommon.getReader(input.getPath(), WorkerCommon.s3))
                {
                    ...
                    PixelsReaderOption option = WorkerCommon.getReaderOption(queryId, leftCols, input);
