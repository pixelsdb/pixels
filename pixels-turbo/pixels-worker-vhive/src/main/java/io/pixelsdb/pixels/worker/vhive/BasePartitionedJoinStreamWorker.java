...
public class BasePartitionedJoinStreamWorker extends Worker<PartitionedJoinInput, JoinOutput>
{
    ...
    public JoinOutput process(PartitionedJoinInput event)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < leftPartitioned.size(); i += leftSplitSize)
            {
                ...
                // todo: maybe allow 1 join worker to use multiple ports to read input in parallel, so as to build the hash table in parallel.
                ...
                leftFutures.add(threadPool.submit(() -> {
                    try
                    {
                        buildHashTable(transId, joiner, parts, leftColumnsToRead, leftInputStorageInfo.getScheme(),
                                hashValues, 1, workerMetrics);  // todo: hardcoded
                    }
                    catch (Throwable e)
                    {
                        ...
                        try
                        {
                            int numJoinedRows = partitionOutput ?
                                    joinWithRightTableAndPartition(
                                            ...
                                            5, outputPartitionInfo, result, workerMetrics) :
                                    joinWithRightTable(transId, joiner, parts, rightColumnsToRead,
                                            rightInputStorageInfo.getScheme(), hashValues, 5,
                                            result.get(0), workerMetrics);  // todo: hardcoded
                        } catch (Throwable e)
                        {
            ...
            try
            {
                ...
                if (partitionOutput)
                {
                    pixelsWriter = StreamWorkerCommon.getWriter(joiner.getJoinedSchema(),
                            ...
                            encoding, true, -1, Arrays.stream(  // todo: parse the operator name
                                            outputPartitionInfo.getKeyColumnIds()).boxed().
                                    collect(Collectors.toList()));  // XXX: Should use multi output
     ...
     * @param numHashes the total number of partitions
    ...
    protected static void buildHashTable(long transId, Joiner joiner, List<String> leftParts, String[] leftCols,
                                         Storage.Scheme leftScheme, List<Integer> hashValues, int numHashes,
        ...
        while (!leftParts.isEmpty())
        {
            for (Iterator<String> it = leftParts.iterator(); it.hasNext(); )
            {
                ...
                try
                {
                    pixelsReader = StreamWorkerCommon.getReader(leftScheme, leftPartitioned, true, numHashes);
                    readCostTimer.stop();
                    checkArgument(pixelsReader.isPartitioned(), "pixels file is not partitioned");
                    // TODO
                    Set<Integer> leftHashValues = new HashSet<>(numHashes);
                    for (int hashValue = 0; hashValue < numHashes; ++hashValue)  // XXX: Should this be numPartition or numShards?
                    {
                        leftHashValues.add(hashValue);
                    }
...
//                    }
                    ...
                    // XXX: Currently, PixelsRecordReaderStreamImpl is not thread-safe, so we can only use one instance
    ...
    protected static int joinWithRightTable(
            ...
            List<Integer> hashValues, int numHashes, ConcurrentLinkedQueue<VectorizedRowBatch> joinResult,
            WorkerMetrics workerMetrics) throws IOException {
        int joinedRows = 0;
        WorkerMetrics.Timer readCostTimer = new WorkerMetrics.Timer();
        WorkerMetrics.Timer computeCostTimer = new WorkerMetrics.Timer();
        ...
        while (!rightParts.isEmpty())
        {
            for (Iterator<String> it = rightParts.iterator(); it.hasNext(); )
            {
                ...
                try
                {
                    pixelsReader = StreamWorkerCommon.getReader(rightScheme, rightPartitioned, true, numHashes);
                    readCostTimer.stop();
                    checkArgument(pixelsReader.isPartitioned(), "pixels file is not partitioned");
                    Set<Integer> rightHashValues = new HashSet<>(numHashes);
                    for (int hashValue = 0; hashValue < numHashes; ++hashValue)
                    {
    ...
    protected static int joinWithRightTableAndPartition(
        ...
        while (!rightParts.isEmpty())
        {
            for (Iterator<String> it = rightParts.iterator(); it.hasNext(); )
            {
                ...
                try
                {
                    pixelsReader = StreamWorkerCommon.getReader(rightScheme, rightPartitioned, true, numPartition);
                    readCostTimer.stop();
                    checkArgument(pixelsReader.isPartitioned(), "pixels file is not partitioned");
