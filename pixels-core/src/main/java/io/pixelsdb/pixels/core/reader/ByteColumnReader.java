...
import io.pixelsdb.pixels.core.encoding.RunLenByteDecoder;
...
/**
 * Column reader for byte / tinyint columns.
 *
 * @author guodong
 * @author hank
 */
...
{
    private RunLenByteDecoder decoder;
                     ...
                     ColumnVector vector, PixelsProto.ColumnChunkIndex chunkIndex) throws IOException
    {
        ...
        if (offset == 0)
        {
            ...
            decoder = new RunLenByteDecoder(inputStream);
            // isNull
        ...
        for (int i = vectorIndex; numLeft > 0; )
        {
            ...
            else
            {
                if (nullsPadding)
                {
                    for (int j = i; j < i + numToRead; ++j)
                    {
                        columnVector.vector[j] = inputBuffer.get();
                    }
                }
                else
                {
