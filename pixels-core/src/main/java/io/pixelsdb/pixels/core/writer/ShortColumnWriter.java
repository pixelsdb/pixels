...
/**
 * This is the column writer for short (int16) columns.
 * 
 * @author gengdy
 * @created 2026-08-07
 */
public class ShortColumnWriter extends BaseColumnWriter
{
    private final short[] curPixelVector = new short[pixelStride];
    private final boolean runlengthEncoding;

    public ShortColumnWriter(TypeDescription type, PixelsWriterOption writerOption)
    {
    ...
    private void writeCurPart(ShortColumnVector vector, int length, int offset)
    {
    ...
    void newPixel() throws IOException
    {
        ...
        {
            ByteBuffer buffer = ByteBuffer.allocate(curPixelVectorIndex * Short.BYTES).order(byteOrder);
            for (int i = 0; i < curPixelVectorIndex; i++)
            {
                buffer.putShort(curPixelVector[i]);
                pixelStatRecorder.updateInteger(curPixelVector[i], 1);
            }
