...
/**
 * This is the column writer for integer (int32) columns.
 * 
 * @author gengdy
 * @create 2026-08-07
 */
public class IntColumnWriter extends BaseColumnWriter
{
    private final int[] curPixelVector = new int[pixelStride];
    private final boolean runlengthEncoding;

    public IntColumnWriter(TypeDescription type, PixelsWriterOption writerOption)
    {
    ...
    private void writeCurPart(IntColumnVector vector, int length, int offset)
    {
    ...
    void newPixel() throws IOException
    {
        ...
        {
            ByteBuffer buffer = ByteBuffer.allocate(curPixelVectorIndex * Integer.BYTES).order(byteOrder);
            for (int i = 0; i < curPixelVectorIndex; i++)
            {
                buffer.putInt(curPixelVector[i]);
                pixelStatRecorder.updateInteger(curPixelVector[i], 1);
            }
