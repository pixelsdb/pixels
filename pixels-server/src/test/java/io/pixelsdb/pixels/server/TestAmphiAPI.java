...
public class TestAmphiAPI
{
    ...
    {
        String tpchTrinoQuery22 = "SELECT cntrycode, COUNT(*) AS numcust, SUM(c_acctbal) AS totacctbal FROM (SELECT SUBSTRING(c_phone, 1, 2) AS cntrycode, " +
                "c_acctbal FROM CUSTOMER WHERE SUBSTRING(c_phone, 1, 2) IN ('20', '40', '22', '30', '39', '42', '21') AND c_acctbal > (SELECT AVG(c_acctbal) " +
                "FROM CUSTOMER WHERE c_acctbal > 0.00 AND SUBSTRING(c_phone, 1, 2) IN ('20', '40', '22', '30', '39', '42', '21')) AND NOT EXISTS(SELECT * " +
                "FROM ORDERS WHERE o_custkey = c_custkey)) AS custsale GROUP BY cntrycode ORDER BY cntrycode NULLS FIRST";
        String tpchDuckdbQuery22 = "SELECT cntrycode, COUNT(*) AS numcust, SUM(c_acctbal) AS totacctbal FROM (SELECT SUBSTRING(c_phone, 1, 2) AS cntrycode, " +
                "c_acctbal FROM CUSTOMER WHERE SUBSTRING(c_phone, 1, 2) IN ('20', '40', '22', '30', '39', '42', '21') AND c_acctbal > (SELECT AVG(c_acctbal) " +
                "FROM CUSTOMER WHERE c_acctbal > 0.00 AND SUBSTRING(c_phone, 1, 2) IN ('20', '40', '22', '30', '39', '42', '21')) AND NOT EXISTS(SELECT * FROM " +
                "ORDERS WHERE o_custkey = c_custkey)) AS custsale GROUP BY cntrycode ORDER BY cntrycode";
    ...
    {
        String simpleQuery = "select l_returnflag, l_linestatus, sum(l_quantity) as sum_qty, sum(l_extendedprice) as sum_base_price, " +
                "sum(l_extendedprice *(1 - l_discount)) as sum_disc_price, sum(l_extendedprice * (1 - l_discount) * (1 + l_tax)) as sum_charge," +
                " avg(l_quantity) as avg_qty, avg(l_extendedprice) as avg_price, avg(l_discount) as avg_disc, count(*) as count_order from LINEITEM " +
                "where l_shipdate <= date '1998-12-01' - interval '108' day(3) group by l_returnflag, l_linestatus order by l_returnflag, l_linestatus";

        SqlglotExecutor executor = new SqlglotExecutor();
        List<String> columnList = new ArrayList<>();
        columnList = executor.parseColumnFields(simpleQuery);

        Set<String> expectedColumns = new HashSet<>(Arrays.asList("l_returnflag", "l_linestatus", "l_returnflag", "l_linestatus", "l_quantity", "l_extendedprice",
                "l_quantity", "l_extendedprice", "l_discount", "l_shipdate", "l_returnflag", "l_linestatus",
                "l_extendedprice", "l_extendedprice", "l_discount", "l_tax", "l_discount"));
    ...
    {
        String tpchQuery6 = "select sum(l_extendedprice * l_discount) as revenue from LINEITEM where l_shipdate >= date '1994-01-01' " +
