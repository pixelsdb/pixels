...
-- -----------------------------------------------------
-- Table `pixels_metadata`.`REGIONS`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pixels_metadata`.`REGIONS` ;

CREATE TABLE IF NOT EXISTS `pixels_metadata`.`REGIONS` (
  `REGION_ID` INT NOT NULL,
  `TBLS_TBL_ID` INT NOT NULL,
  PRIMARY KEY (`REGION_ID`),
  INDEX `fk_REGIONS_TBLS1_idx` (`TBLS_TBL_ID` ASC) VISIBLE,
  CONSTRAINT `fk_REGIONS_TBLS1`
    FOREIGN KEY (`TBLS_TBL_ID`)
    REFERENCES `pixels_metadata`.`TBLS` (`TBL_ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
CREATE TABLE IF NOT EXISTS `pixels_metadata`.`LAYOUTS` (
  ...
  INDEX `fk_LAYOUTS_REGIONS1_idx` (`REGIONS_REGION_ID` ASC) VISIBLE,
  CONSTRAINT `fk_LAYOUT_TBLS1`
    FOREIGN KEY (`TBLS_TBL_ID`)
    REFERENCES `pixels_metadata`.`TBLS` (`TBL_ID`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_LAYOUTS_REGIONS1`
    FOREIGN KEY (`REGIONS_REGION_ID`)
    REFERENCES `pixels_metadata`.`REGIONS` (`REGION_ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8
...
-- -----------------------------------------------------
-- Table `pixels_metadata`.`ROWGROUPS`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `pixels_metadata`.`ROWGROUPS` ;

CREATE TABLE IF NOT EXISTS `pixels_metadata`.`ROWGROUPS` (
  `RG_ID` INT NOT NULL,
  `LAYOUTS_LAYOUT_ID` INT NOT NULL,
  `FILE_PATH` VARCHAR(400) NULL,
  `FILE_RG_IDX` INT NULL,
  `IS_WRITE_BUFFER` TINYINT(1) NULL,
  PRIMARY KEY (`RG_ID`),
  INDEX `fk_ROWGROUPS_LAYOUTS1_idx` (`LAYOUTS_LAYOUT_ID` ASC) VISIBLE,
  CONSTRAINT `fk_ROWGROUPS_LAYOUTS1`
