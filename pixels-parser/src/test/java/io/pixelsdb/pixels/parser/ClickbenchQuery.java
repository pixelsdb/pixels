...
public class ClickbenchQuery
{
    ...
    public static final String Q28 = "SELECT counterid, AVG(CHAR_LENGTH(url)) AS l, COUNT(*) AS c FROM hits WHERE url <> '' GROUP BY counterid HAVING COUNT(*) > 100000 ORDER BY l DESC LIMIT 25";
    public static final String Q29 = "SELECT REGEXP_REPLACE(referer, '^https?://(?:www\\.)?([^/]+)/.*$', '\1') AS k, AVG(length(referer)) AS l, COUNT(*) AS c, MIN(referer) FROM hits WHERE referer <> '' GROUP BY REGEXP_REPLACE(referer, '^https?://(?:www\\.)?([^/]+)/.*$', '\1') HAVING COUNT(*) > 100000 ORDER BY l DESC LIMIT 25";
