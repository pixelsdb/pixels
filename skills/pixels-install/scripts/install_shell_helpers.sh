...
PIXELS_DEFAULT_HOME=$default_pixels_home

_pixels_home() {
  printf '%s\n' "\${PIXELS_HOME:-\$PIXELS_DEFAULT_HOME}"
}
...
install_remote_coordinator() {
  ...
  ssh "${ssh_args[@]}" "$remote" "chmod +x $(shell_quote "$REMOTE_SCRIPT_DIR/install_shell_helpers.sh") && command -v bash >/dev/null 2>&1 && env $ssh_target bash $(shell_quote "$REMOTE_SCRIPT_DIR/install_shell_helpers.sh")"
}
