...
# symlink swaps across upgrades - matching the official recommendation to
# keep the data dir outside the install dir.
...
persist_trino_pixels_environment() {
  local profile_file profile_pixels_home profile_pixels_config

  profile_file="$(detect_profile_file)"
  profile_pixels_home="$(profile_path_value "$TRINO_PIXELS_HOME")"
  if [[ "$TRINO_PIXELS_CONFIG" == "$TRINO_PIXELS_HOME/etc/pixels.properties" ]]; then
    profile_pixels_config='$PIXELS_HOME/etc/pixels.properties'
  else
    profile_pixels_config="$(profile_path_value "$TRINO_PIXELS_CONFIG")"
  fi

  persist_export "$profile_file" PIXELS_HOME "$profile_pixels_home"
  persist_export "$profile_file" PIXELS_CONFIG "$profile_pixels_config"
  persist_toolchain_var PIXELS_HOME "$TRINO_PIXELS_HOME"
  persist_toolchain_var PIXELS_CONFIG "$TRINO_PIXELS_CONFIG"

  export PIXELS_HOME="$TRINO_PIXELS_HOME"
  export PIXELS_CONFIG="$TRINO_PIXELS_CONFIG"
  log "persisted PIXELS_HOME/PIXELS_CONFIG for Trino in $profile_file"
}
...
verify_install() {
  ...
  if [[ "$INSTALL_PIXELS_TRINO_PLUGIN" == "true" ]]; then
    ...
    grep -qE '^export PIXELS_HOME=' "$(detect_profile_file)" || fail "PIXELS_HOME export not written to $(detect_profile_file)"
    grep -qE '^export PIXELS_CONFIG=' "$(detect_profile_file)" || fail "PIXELS_CONFIG export not written to $(detect_profile_file)"
    if [[ "$INSTALL_PIXELS_TRINO_LISTENER" == "true" ]]; then
