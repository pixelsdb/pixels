...
- `install_shell_helpers.sh`: optional convenience step that **asks first** before writing shell functions into the current user's profile. Writes `start_pixels`/`stop_pixels`/`restart_pixels`/`redeploy_pixels` plus optional stack helpers (`start_pixels_stack`, `stop_pixels_stack`, `restart_pixels_stack`, `redeploy_pixels_stack`) to `~/.pixels-shell-helpers.sh`; stack helpers call Trino cluster helpers only if those functions are already installed in the shell. Leave `INSTALL_PIXELS_SHELL_HELPERS` unset to be prompted `[y/N]`; set `true`/`false` to answer non-interactively once the user has actually agreed, or `ASSUME_YES=true`.
- `smoke_test.sh`: verify the installed layout, configuration, Java (`CHECK_JAVA=true`), metadata access, etcd health, Pixels topology (`deployment.env` plus `$PIXELS_HOME/etc/workers`), core service ports (`CHECK_CORE_SERVICES=true`), `$PIXELS_HOME/logs` error patterns (`CHECK_PIXELS_LOGS=true`), and basic CLI behavior when applicable. It also checks Trino layout/catalog files when `CHECK_TRINO=true` and `trino-deployment.env` is present, can separately validate the lightweight Trino-side Pixels client config (`CHECK_TRINO_PIXELS_CLIENT=true`) without requiring a full Pixels runtime layout (`CHECK_PIXELS_LAYOUT=false`), can check Trino launcher status across the recorded cluster (`CHECK_TRINO_CLUSTER_STATUS=true`), and can run `trino --catalog pixels --execute "SHOW SCHEMAS"` (`CHECK_TRINO_CLI=true`). `SHOW SCHEMAS` only needs to succeed; an empty Pixels catalog is expected before data is loaded. Same structured-summary behavior as `check_prerequisites.sh` above.
...
- **install_shell_helpers_optional**: only if the user wants Pixels convenience functions; `install_shell_helpers.sh` asks by default before writing to the user's shell profile.
...
- Do not wrap or replace Trino's `bin/launcher` to smuggle Pixels environment variables. Use the generated user-scoped `~/.pixels-trino-env.sh` and source it from shell helpers or the user's shell profile.
